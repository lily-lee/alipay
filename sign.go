...
// @Param privateKey []byte 加签私钥(pem.Decode()后得到的p.Bytes)
func RsaSign(content, signType string, privateKey []byte) (string, error) {
	...
	keyI, err := x509.ParsePKCS8PrivateKey(privateKey)
	if err != nil {
...
// @Param sign_type string 签名方式
func RsaSignFromStrKey(content, signType, privateKey string) (string, error) {
	hash, err := getHashType(signType)
	if err != nil {
		return "", err
	}
...
// RsaCheck 此方法会去掉sign_type做验签，暂时除生活号（原服务窗）激活开发者模式外都使用V1。
// @param params map[string]string 参数列表(包括待验签参数和签名值sign) key-参数名称 value-参数值
// @Param sign_type string 签名方式
// @param publicKey []byte 验签公钥(pem.Decode()后得到的p.Bytes)
func RsaCheck(params map[string]string, signType string, publicKey []byte) error {
	sign, ok := params["sign"]
	if !ok {
	...
	keyI, err := x509.ParsePKIXPublicKey(publicKey)
	if err != nil {
...
// RsaCheckFromStrKey 此方法会去掉sign_type做验签，暂时除生活号（原服务窗）激活开发者模式外都使用V1。
// @param params map[string]string 参数列表(包括待验签参数和签名值sign) key-参数名称 value-参数值
// @param publicKey string 验签公钥
// @Param sign_type string 签名方式
func RsaCheckFromStrKey(params map[string]string, signType, publicKey string) error {
	sign, ok := params["sign"]
	if !ok {
		return errors.New("sign is needed")
	}
...
// @Param sign_type string 签名方式
func RsaCheckV2(params map[string]string, signType, publicKey string) bool {
	return true
}
...
func ReadKeyFromPemFile(filePath string) ([]byte, error) {
